...
db.transaction((tx) => {
  ...
  tx.executeSql(
    "CREATE TABLE IF NOT EXISTS Lista (codigo INTEGER PRIMARY KEY AUTOINCREMENT, descricao TEXT);"
  );
});
...
const create = (obj) => {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      ...
      tx.executeSql(
        "INSERT INTO Lista (descricao) values (?);",
        [obj.descricao],
        //-----------------------
...
const update = (id, obj) => {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      //comando SQL modificável
      tx.executeSql(
        "UPDATE lista SET descricao=? WHERE codigo=?;",
        [obj.descricao, id],
        //-----------------------
        (_, { rowsAffected }) => {
...
const remove = (id) => {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      //comando SQL modificável
      tx.executeSql(
        "DELETE FROM lista WHERE codigo=?;",
        [id],
        //-----------------------
        (_, { rowsAffected }) => {
